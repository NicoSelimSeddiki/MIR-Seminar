...
% y = resample(x,tx,fs,p,q) interpolates the input signal to an 
% intermediate uniform grid with a sample spacing of (p/q)/fs.
%
% The function then filters the result to upsample it by p and downsample it by q,
% resulting in a final sample rate of fs. For best results, ensure that fs × q/p
% is at least twice as large as the highest frequency component of x.
%
kl = (B*log2(1+1/d));
...
%% Extracting Peaks
[~, num_times] = size(Ures_LogP_absCQT);

% Get the indices where the frequency is less or equal than 1000Hz
I = find(Ures_FreqVec >= 50 & Ures_FreqVec <= 4000)';
Ures_FreqVec = Ures_FreqVec(I);

Ures_LogP_absCQT = Ures_LogP_absCQT(I, :); 
Ures_LogP_absCQT = detrend(Ures_LogP_absCQT);
Ures_LogP_absCQT = Ures_LogP_absCQT ./ max(Ures_LogP_absCQT);

% For each time instance, extract the peaks
outer = struct([]);
for i=1:num_times

    % Get the peaks of the regular log spec
    [~, locs] = findpeaks(Ures_LogP_absCQT(:, i) , ...
        'MinPeakProminence', 0, ... 
        'SortStr', ...
        'descend');

    % Number of the most 10 prominent peaks
    locs = sort(locs(1:10));
    l = find(max(Ures_LogP_absCQT(:, i)) == Ures_LogP_absCQT(:, i));
    locs = locs(find(locs == l):end);

    % Define struct with key value pairs
    % https://de.mathworks.com/matlabcentral/answers/304616-inserting-fields-into-an-empty-structure
    inner = struct('locations', locs);
    outer(i).i = inner;
end     

% Copy the values of LogO_absCQT_I1 at locs1 into the new array
interp_spec = zeros(size(Ures_LogP_absCQT));

% Copy the highest bins into the array
locs = outer(1).i.locations;
for i = 1:num_times
    interp_spec(locs, i) = Ures_LogP_absCQT(locs, i);
end    

% Copy bin with pre-defined bandwidth
for i = 1:length(locs)
    kth_frq = locs(i);

    lower_indices = Ures_FreqVec > (Ures_FreqVec(kth_frq) - 60) & ...
        Ures_FreqVec < Ures_FreqVec(kth_frq);

    upper_indices =  Ures_FreqVec < (Ures_FreqVec(kth_frq) + 60) & ...
        Ures_FreqVec > Ures_FreqVec(kth_frq);

    interp_spec(lower_indices) = Ures_LogP_absCQT(lower_indices);
    interp_spec(upper_indices) = Ures_LogP_absCQT(upper_indices);
end
interp_spec(interp_spec < 0) = 0;

%% Shift into baseband!!!
% This actually just shifts the fequency vector into the based
Ures_FreqVec = Ures_FreqVec / Ures_FreqVec(1);

%% DCT
% CQcepstrum = dct(Ures_LogP_absCQT);
CQcepstrum = dct(interp_spec);
